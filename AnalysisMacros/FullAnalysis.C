 ...
 // ---------------------- Acceptance and Efficiency Invariant Mass  -----------------------------
  ...
  invMassAccEffNum->Draw("ep");
	...
	for(Int_t i =0; i < 7; i++)
	{
		...
		if(i == 0) invMassCorrectedRecoNum->SetTitle("Invariant Mass - (DATA / AE ) / GEN");
...
//	------------------------ CORRECTED DATA --------------------------------------------
	TCanvas *correctedInvMassCanvasDATADATA = new TCanvas("correctedInvMassCanvasDATA2", "Invariant Mass - (MC / AE ) / GEN ", canvasXResolution,canvasYResolution);
	...
	histogramsInvariantMassMC[0]->Draw("");
